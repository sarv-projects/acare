/*
 * RMCS-3002 — 3-Motor Modbus ASCII Open-Loop Controller
 * ======================================================
 * Source: Shreyas EC (team member)
 * Status: UNTESTED — provided as reference only.
 *          Not integrated into the main ACARE Teensy firmware.
 *
 * Hardware:
 *   - Serial2 → SLAVE_ID1 (Motor 1)
 *   - Serial3 → SLAVE_ID2 (Motor 2)
 *   - Serial4 → SLAVE_ID3 (Motor 3)
 *
 * Commands over Serial (115200 baud):
 *   M1 CW <pwm>   — Motor 1 clockwise at PWM (0–4800)
 *   M1 CCW <pwm>  — Motor 1 counter-clockwise at PWM
 *   M1 B          — Motor 1 brake
 *   M2 CW <pwm>   — Motor 2 clockwise
 *   M2 CCW <pwm>  — Motor 2 counter-clockwise
 *   M2 B          — Motor 2 brake
 *   M3 CW <pwm>   — Motor 3 clockwise
 *   M3 CCW <pwm>  — Motor 3 counter-clockwise
 *   M3 B          — Motor 3 brake
 */
...
#define SLAVE_ID3 0x03

#define motorSerial1 Serial2
#define motorSerial2 Serial3
#define motorSerial3 Serial4
...
{
  Serial.begin(115200);
  motorSerial1.begin(9600);
  motorSerial2.begin(9600);
  motorSerial3.begin(9600);

  Serial.println("System start");
  Serial.println("Applying initial brake (arming drives)");

  // arm controllers immediately
  motorBrake(SLAVE_ID1);
  motorBrake(SLAVE_ID2);
  motorBrake(SLAVE_ID3);
  ...
  Serial.println("  M2 B");
  Serial.println("  M3 CW <pwm>");
  Serial.println("  M3 CCW <pwm>");
  Serial.println("  M3 B");
}
...
void loop()
{
  ...
  uint8_t motorId =
    ...
    motorStr.equalsIgnoreCase("M2") ? SLAVE_ID2 :
    motorStr.equalsIgnoreCase("M3") ? SLAVE_ID3 : 0;
